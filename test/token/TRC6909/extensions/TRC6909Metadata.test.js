...
async function fixture() {
  const token = await ethers.deployContract('$TRC6909Metadata');
  return { token };
}

describe('TRC6909Metadata', function () {
  ...
  describe('name', function () {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setName(1n, 'My Token'))
        .to.emit(this.token, 'TRC6909NameUpdated')
  ...
  describe('symbol', function () {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setSymbol(1n, 'MTK')).to.emit(this.token, 'TRC6909SymbolUpdated').withArgs(1n, 'MTK');
  ...
  describe('decimals', function () {
    ...
    it('can be set by dedicated setter', async function () {
      await expect(this.token.$_setDecimals(1n, 18)).to.emit(this.token, 'TRC6909DecimalsUpdated').withArgs(1n, 18);
  ...
  shouldSupportInterfaces(['TRC6909', 'TRC6909Metadata']);
});
