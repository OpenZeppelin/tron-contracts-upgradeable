...
// TIP-721 magic value — `bytes4(keccak256("onTRC721Received(address,address,uint256,bytes)"))`.
// Differs from EIP-721's `0x150b7a02`.
const RECEIVER_MAGIC_VALUE = '0x5175f878';

function shouldBehaveLikeTRC721() {
  beforeEach(async function () {
    const [owner, newOwner, approved, operator, other] = this.accounts;
    Object.assign(this, { owner, newOwner, approved, operator, other });
  });

  shouldSupportInterfaces(['TRC721']);
  ...
  describe('with minted tokens', function () {
      ...
      describe('when querying the zero address', function () {
        it('throws', async function () {
          await expect(this.token.balanceOf(ethers.ZeroAddress))
            .to.be.revertedWithCustomError(this.token, 'TRC721InvalidOwner')
      ...
      describe('when the given token ID was not tracked by this token', function () {
        ...
        it('reverts', async function () {
          await expect(this.token.ownerOf(tokenId))
            .to.be.revertedWithCustomError(this.token, 'TRC721NonexistentToken')
            .withArgs(tokenId);
        });
      });
    });
    ...
    describe('transfers', function () {
        ...
        describe('when the address of the previous owner is incorrect', function () {
          it('reverts', async function () {
            await expect(
              ...
              .to.be.revertedWithCustomError(this.token, 'TRC721IncorrectOwner')
          ...
          if (opts.unrestricted) {
            ...
            it('reverts', async function () {
              await expect(
                ...
                .to.be.revertedWithCustomError(this.token, 'TRC721InsufficientApproval')
        ...
        describe('when the address to transfer the token to is the zero address', function () {
          it('reverts', async function () {
            await expect(
              ...
              .to.be.revertedWithCustomError(this.token, 'TRC721InvalidReceiver')
              .withArgs(ethers.ZeroAddress);
          });
        });
      };
      ...
      const shouldTransferSafely = function (fragment, data, opts = {}) {
        ...
        describe('to a valid receiver contract', function () {
          beforeEach(async function () {
            this.to = await ethers.deployContract('TRC721ReceiverMock', [RECEIVER_MAGIC_VALUE, RevertType.None]);
          });
      ...
      for (const { fnName, opts } of [
        ...
        describe(`via ${fnName}`, function () {
          ...
          describe('to a receiver contract returning unexpected value', function () {
            it('reverts', async function () {
              const invalidReceiver = await ethers.deployContract('TRC721ReceiverMock', [
                '0xdeadbeef',
                RevertType.None,
              ]);
          ...
          describe('to a receiver contract that reverts with message', function () {
            it('reverts', async function () {
              const revertingReceiver = await ethers.deployContract('TRC721ReceiverMock', [
              ...
              ).to.be.revertedWith('TRC721ReceiverMock: reverting');
            });
          });
    ...
    describe('safe mint', function () {
      ...
      describe('via safeMint', function () {
        // regular minting is tested in TRC721Mintable.test.js and others
        it('calls onTRC721Received — with data', async function () {
          const receiver = await ethers.deployContract('TRC721ReceiverMock', [RECEIVER_MAGIC_VALUE, RevertType.None]);
        ...
        describe('to a receiver contract returning unexpected value', function () {
          it('reverts', async function () {
            const invalidReceiver = await ethers.deployContract('TRC721ReceiverMock', ['0xdeadbeef', RevertType.None]);
      ...
      describe('when the sender does not own the given token ID', function () {
        it('reverts', async function () {
          await expect(this.token.connect(this.other).approve(this.approved, tokenId))
            .to.be.revertedWithCustomError(this.token, 'TRC721InvalidApprover')
            .withArgs(this.other);
        });
      });
      ...
      describe('when the operator is address zero', function () {
        it('reverts', async function () {
          await expect(this.token.connect(this.owner).setApprovalForAll(ethers.ZeroAddress, true))
            .to.be.revertedWithCustomError(this.token, 'TRC721InvalidOperator')
            .withArgs(ethers.ZeroAddress);
        });
      });
    ...
    describe('with minted token', function () {
      ...
      it('reverts when adding a token id that already exists', async function () {
        await expect(this.token.$_mint(this.owner, firstTokenId))
          .to.be.revertedWithCustomError(this.token, 'TRC721InvalidSender')
          .withArgs(ethers.ZeroAddress);
      });
    });
  });
...
function shouldBehaveLikeTRC721Enumerable() {
  beforeEach(async function () {
    const [owner, newOwner, approved, operator, other] = this.accounts;
    Object.assign(this, { owner, newOwner, approved, operator, other });
  });

  shouldSupportInterfaces(['TRC721Enumerable']);
      ...
      describe('when the index is greater than or equal to the total tokens owned by the given address', function () {
        it('reverts', async function () {
          await expect(this.token.tokenOfOwnerByIndex(this.owner, 2n))
            .to.be.revertedWithCustomError(this.token, 'TRC721OutOfBoundsIndex')
...
function shouldBehaveLikeTRC721Metadata(name, symbol) {
  shouldSupportInterfaces(['TRC721Metadata']);
