...
describe('TRC721Consecutive', function () {
  for (const offset of [0n, 1n, 42n]) {
    describe(`with offset ${offset}`, function () {
      async function fixture() {
        ...
        const token = await ethers.deployContract('$TRC721ConsecutiveMock', [
          name,
          symbol,
          offset,
          delegates,
      ...
      describe('minting during construction', function () {
        ...
        it('reverts on consecutive minting to the zero address', async function () {
          await expect(
            ethers.deployContract('$TRC721ConsecutiveMock', [
              name,
              symbol,
              offset,
              this.delegates,
      ...
      describe('TRC721 behavior', function () {
  ...
  describe('invalid use', function () {
    ...
    it('cannot mint a batch larger than 5000', async function () {
      const factory = await ethers.getContractFactory('$TRC721ConsecutiveMock');

      await expect(ethers.deployContract('$TRC721ConsecutiveMock', [name, symbol, 0, [], [receiver], [5001n]]))
        .to.be.revertedWithCustomError(factory, 'TRC721ExceededMaxBatchMint')
        .withArgs(5001n, 5000n);
    });

    it('cannot use single minting during construction', async function () {
      const factory = await ethers.getContractFactory('$TRC721ConsecutiveNoConstructorMintMock');

      await expect(
        ethers.deployContract('$TRC721ConsecutiveNoConstructorMintMock', [name, symbol]),
      ).to.be.revertedWithCustomError(factory, 'TRC721ForbiddenMint');
    });

    it('consecutive mint not compatible with enumerability', async function () {
      const factory = await ethers.getContractFactory('$TRC721ConsecutiveEnumerableMock');

      await expect(
        ethers.deployContract('$TRC721ConsecutiveEnumerableMock', [name, symbol, [receiver], [100n]]),
      ).to.be.revertedWithCustomError(factory, 'TRC721EnumerableForbiddenBatchMint');
    });
  });
});
