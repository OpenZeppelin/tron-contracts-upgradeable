...
const { shouldBehaveLikeTRC721 } = require('../TRC721.behavior');
...
async function fixture() {
  ...
  const underlying = await ethers.deployContract('$TRC721', [name, symbol]);
  await underlying.$_safeMint(owner, tokenId);
  await underlying.$_safeMint(owner, otherTokenId);
  const token = await ethers.deployContract('$TRC721Wrapper', [`Wrapped ${name}`, `W${symbol}`, underlying]);

  return { accounts, owner, approved, other, underlying, token };
}

describe('TRC721Wrapper', function () {
  ...
  describe('onTRC721Received', function () {
    it('only allows calls from underlying', async function () {
      await expect(
        this.token.connect(this.other).onTRC721Received(
          this.owner,
          this.token,
          tokenId,
        ...
        .to.be.revertedWithCustomError(this.token, 'TRC721UnsupportedToken')
        .withArgs(this.other);
    });
  ...
  describe('_recover', function () {
    it('works if there is something to recover', async function () {
      // Should use `transferFrom` to avoid `onTRC721Received` minting
