...
async function fixture() {
  ...
  const receiver = await ethers.deployContract('TRC1363ReceiverMock');
  const spender = await ethers.deployContract('TRC1363SpenderMock');
  const token = await ethers.deployContract('$TRC1363', [name, symbol]);
...
describe('TRC1363', function () {
  beforeEach(async function () {
    Object.assign(this, await loadFixture(fixture));
  });

  shouldSupportInterfaces(['TRC165', 'TRC1363']);
  ...
  describe('transferAndCall', function () {
    ...
    it('reverts with reverting hook (with reason)', async function () {
      ...
      ).to.be.revertedWith('TRC1363ReceiverMock: reverting');
    });
  ...
  describe('approveAndCall', function () {
    ...
    it('reverts approving an EOA', async function () {
      await expect(this.token.connect(this.holder).getFunction('approveAndCall(address,uint256)')(this.other, value))
        .to.be.revertedWithCustomError(this.token, 'TRC1363InvalidSpender')
        .withArgs(this.other.address);
    });
    ...
    it('reverts with reverting hook (with reason)', async function () {
      ...
      await expect(
        this.token.connect(this.holder).getFunction('approveAndCall(address,uint256,bytes)')(this.spender, value, data),
      ).to.be.revertedWith('TRC1363SpenderMock: reverting');
    });
