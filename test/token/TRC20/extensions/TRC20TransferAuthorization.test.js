...
describe('TRC20TransferAuthorization', function () {
  for (const mode of ['timestamp', 'blocknumber']) {
    const fixture = async () => {
      ...
      const token = await ethers.deployContract('$TRC20TransferAuthorization', [name, symbol, name, version]);
      await token.$_mint(holder, initialSupply);

      const wallet = await ethers.deployContract('TRC1271WalletMock', [holder]);
    ...
    describe(`with ${mode}`, function () {
      ...
      describe('transferWithAuthorization', function () {
        ...
        it('rejects other signature', async function () {
          ...
          ).to.be.revertedWithCustomError(this.token, 'TRC3009InvalidSignature');
        });
        ...
        it('rejects authorization not yet valid', async function () {
          ...
          await expect(
            ...
            .to.be.revertedWithCustomError(this.token, 'TRC3009InvalidAuthorizationTime')
        ...
        describe('with bytes signature', function () {
          ...
          it('accepts TRC1271 wallet signature', async function () {
          ...
          it('rejects invalid TRC1271 signature', async function () {
            const signature = await getDomain(this.token).then(domain =>
      ...
      describe('receiveWithAuthorization', function () {
        ...
        describe('with bytes signature', function () {
          ...
          it('accepts TRC1271 wallet signature when called by recipient', async function () {
            const signature = await getDomain(this.token).then(domain =>
              this.holder.signTypedData(
                domain,
                { ReceiveWithAuthorization },
      ...
      describe('cancelAuthorization', function () {
        ...
        describe('with bytes signature', function () {
          ...
          it('prevents usage of canceled authorization by a TRC1271 wallet', async function () {
            const value = 42n;
            const validAfter = withFlag(0n, mode);
            const validBefore = withFlag(0x7fffffffffffn, mode);
