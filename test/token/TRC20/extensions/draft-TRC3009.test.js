...
const fixture = async () => {
  ...
  const token = await ethers.deployContract('$TRC3009', [name, symbol, name, version]);
  ...
  for (const mode of ['timestamp', 'blocknumber']) {
    describe(`using ${mode} clock`, function () {
      ...
      describe('transferWithAuthorization', function () {
        ...
        it('rejects reused nonce with TRC3009UsedAuthorization', async function () {
          const { v, r, s } = await getDomain(this.token)
            .then(domain =>
              this.holder.signTypedData(
                domain,
          ...
          await expect(
            ...
            .to.be.revertedWithCustomError(this.token, 'TRC3009UsedAuthorization')
      ...
      describe('cancelAuthorization', function () {
        ...
        it('rejects re-cancellation with TRC3009UsedAuthorization', async function () {
          const { v, r, s } = await getDomain(this.token)
            .then(domain =>
              this.holder.signTypedData(
                domain,
