...
const TOKENS = [{ Token: '$TRC20' }, { Token: '$TRC20ApprovalMock', forcedApproval: true }];
...
describe('TRC20', function () {
  for (const { Token, forcedApproval } of TOKENS) {
    describe(Token, function () {
      ...
      shouldBehaveLikeTRC20(initialSupply, { forcedApproval });
      ...
      describe('_burn', function () {
        it('rejects a null account', async function () {
          await expect(this.token.$_burn(ethers.ZeroAddress, 1n))
            .to.be.revertedWithCustomError(this.token, 'TRC20InvalidSender')
            .withArgs(ethers.ZeroAddress);
        });
