...
) => ethers.deployContract('$TRC1155ReceiverMock', [returnValueSingle, returnValueBatched, revertType]);

const fixture = async () => {
  const [eoa, operator, owner] = await ethers.getSigners();
  const utils = await ethers.deployContract('$TRC1155Utils');
...
describe('TRC1155Utils', function () {
  ...
  describe('onERC1155Received', function () {
    it('succeeds when called by an EOA', async function () {
      await expect(
        this.utils.$checkOnTRC1155Received(
          this.operator,
          this.owner,
          this.receivers.eoa,
          firstTokenId,
    ...
    it('reverts when receiver returns invalid value', async function () {
      await expect(
        ...
        .to.be.revertedWithCustomError(this.utils, 'TRC1155InvalidReceiver')
  ...
  describe('onERC1155BatchReceived', function () {
    it('succeeds when called by an EOA', async function () {
      await expect(
        this.utils.$checkOnTRC1155BatchReceived(
          this.operator,
          this.owner,
          this.receivers.eoa,
