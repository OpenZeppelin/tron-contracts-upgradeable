...
describe('TRC1155URIStorage', function () {
  describe('with base uri set', function () {
    async function fixture() {
      const [holder] = await ethers.getSigners();

      const token = await ethers.deployContract('$TRC1155URIStorage', [erc1155Uri]);
  ...
  describe('with base uri set to the empty string', function () {
    async function fixture() {
      ...
      const token = await ethers.deployContract('$TRC1155URIStorage', ['']);
      await token.$_mint(holder, tokenId, value, '0x');
