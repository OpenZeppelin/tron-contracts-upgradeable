...
const { shouldBehaveLikeBridgeTRC1155 } = require('./BridgeTRC1155.behavior');
...
async function fixture() {
  ...
  // Chain A: legacy TRC1155 with bridge
  const tokenA = await ethers.deployContract('$TRC1155', ['https://token-cdn-domain/{id}.json']);
  const bridgeA = await ethers.deployContract('$BridgeTRC1155', [[], tokenA]);
...
describe('CrosschainBridgeTRC1155', function () {
  ...
  shouldBehaveLikeBridgeTRC1155({ chainAIsCustodial: true });
  ...
  describe('direct transfer to bridge should fail', function () {
    ...
    it('single', async function () {
      ...
      await expect(this.tokenA.connect(alice).safeTransferFrom(alice, this.bridgeA, ids[0], values[0], '0x'))
        .to.be.revertedWithCustomError(this.tokenA, 'TRC1155InvalidReceiver')
