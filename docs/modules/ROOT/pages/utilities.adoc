...
On TRON, the `verify` function tries the native secp256r1 precompile specified by https://github.com/tronprotocol/tips/blob/master/tip-7951.md[TIP-7951] (at address `0x100`) and falls back to the Solidity implementation if the precompile is not available. The precompile activates per network through the `ALLOW_TVM_OSAKA` chain parameter. Use `verifyNative` if you know the precompile is active on your target network (it reverts when it is not), or `verifySolidity` to skip the precompile entirely.
