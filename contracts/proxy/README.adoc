...
In order to avoid clashes with the storage variables of the implementation contract behind a proxy, we use https://github.com/tronprotocol/tips/blob/master/tip-1967.md[TIP-1967] (the TRON-side analogue of https://eips.ethereum.org/EIPS/eip-1967[EIP-1967]) storage slots.
...
- {TRC1967Clones}: A library that can deploy minimal TRC-1967 proxies that replicate the behavior of {TRC1967Proxy} with a bytecode optimized for cheap deployment and usage. Unlike {TRC1967Proxy}, the clones do not accept initialization data at construction, so the deployer is expected to initialize the proxy in the same transaction as the deployment.
...
CAUTION: Using upgradeable proxies correctly and securely is a difficult task that requires deep knowledge of the proxy pattern, Solidity, and the TVM. Unless you want a lot of low level control, we recommend using the Tron Upgrades plugins: https://github.com/OpenZeppelin/tron-upgrades[`@openzeppelin/hardhat-tron-upgrades` and `@openzeppelin/tronbox-upgrades`], or https://github.com/OpenZeppelin/openzeppelin-foundry-upgrades-tron[`@openzeppelin/foundry-upgrades-tron`] for Foundry. The OpenZeppelin Upgrades Plugins for Ethereum are not compatible with Tron.
...
The current implementation of this security mechanism uses TRC-1822, as defined in the https://eips.ethereum.org/EIPS/eip-1822[ERC], to detect the storage slot used by the implementation. A previous implementation, now deprecated, relied on a rollback check. It is possible to upgrade from a contract using the old mechanism to a new one. The inverse is however not possible, as old implementations (before version 4.5) did not include the TRC-1822 interface.
