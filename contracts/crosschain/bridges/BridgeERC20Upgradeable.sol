...
import {ITRC20Upgradeable, SafeTRC20Upgradeable} from "../../token/TRC20/utils/SafeTRC20Upgradeable.sol";
...
abstract contract BridgeERC20Upgradeable is Initializable, BridgeFungibleUpgradeable {
    ...
    function __BridgeERC20_init(ITRC20Upgradeable token_) internal onlyInitializing {
        __BridgeERC20_init_unchained(token_);
    }

    function __BridgeERC20_init_unchained(ITRC20Upgradeable token_) internal onlyInitializing {
        BridgeERC20Storage storage $ = _getBridgeERC20Storage();
        $._token = token_;
    }

    /// @dev Return the address of the TRC20 token this bridge operates on.
    function token() public view virtual returns (ITRC20Upgradeable) {
        BridgeERC20Storage storage $ = _getBridgeERC20Storage();
        return $._token;
    }
