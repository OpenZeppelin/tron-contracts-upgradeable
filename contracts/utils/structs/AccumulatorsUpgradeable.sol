 ...
 */
library AccumulatorsUpgradeable {
     ...
     */
    struct Accumulator {
        MemoryUpgradeable.Pointer head;
        MemoryUpgradeable.Pointer tail;
    }

    /// @dev Item (list node) in a bytes accumulator
    struct AccumulatorEntry {
        MemoryUpgradeable.Pointer next;
        MemoryUpgradeable.Slice data;
    }
    ...
    function push(Accumulator memory self, MemoryUpgradeable.Slice data) internal pure returns (Accumulator memory) {
        if (!data.isReserved()) PanicUpgradeable.panic(PanicUpgradeable.RESOURCE_ERROR);

        MemoryUpgradeable.Pointer ptr = _asPtr(AccumulatorEntry({next: _nullPtr(), data: data}));
    ...
    function shift(Accumulator memory self, MemoryUpgradeable.Slice data) internal pure returns (Accumulator memory) {
        if (!data.isReserved()) PanicUpgradeable.panic(PanicUpgradeable.RESOURCE_ERROR);

        MemoryUpgradeable.Pointer ptr = _asPtr(AccumulatorEntry({next: self.head, data: data}));
    ...
    function _asPtr(AccumulatorEntry memory item) private pure returns (MemoryUpgradeable.Pointer ptr) {
        assembly ("memory-safe") {
            ptr := item
        }
    }

    function _asAccumulatorEntry(MemoryUpgradeable.Pointer ptr) private pure returns (AccumulatorEntry memory item) {
        assembly ("memory-safe") {
            item := ptr
        }
    }

    function _nullPtr() private pure returns (MemoryUpgradeable.Pointer) {
        return MemoryUpgradeable.Pointer.wrap(0);
    }
}
