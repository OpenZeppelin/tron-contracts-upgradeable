...
library EnumerableSetUpgradeable {
    ...
    function _clear(Set storage set) private {
        ...
        ArraysUpgradeable.unsafeSetLength(set._values, 0);
    }
    ...
    function _values(Set storage set, uint256 start, uint256 end) private view returns (bytes32[] memory) {
        unchecked {
            end = MathUpgradeable.min(end, _length(set));
            start = MathUpgradeable.min(start, end);

            uint256 len = end - start;
            bytes32[] memory result = new bytes32[](len);
            for (uint256 i = 0; i < len; ++i) {
                result[i] = ArraysUpgradeable.unsafeAccess(set._values, start + i).value;
            }
            return result;
        }
    }
    ...
    function values(StringSet storage set, uint256 start, uint256 end) internal view returns (string[] memory) {
        unchecked {
            end = MathUpgradeable.min(end, length(set));
            start = MathUpgradeable.min(start, end);
