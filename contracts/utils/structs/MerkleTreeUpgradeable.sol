...
import {HashesUpgradeable} from "../cryptography/HashesUpgradeable.sol";
...
library MerkleTreeUpgradeable {
    ...
    function setup(Bytes32PushTree storage self, uint8 treeDepth, bytes32 zero) internal returns (bytes32 initialRoot) {
        return setup(self, treeDepth, zero, HashesUpgradeable.commutativeKeccak256);
    }
    ...
    function setup(
        ...
        ArraysUpgradeable.unsafeSetLength(self._sides, treeDepth);
        ArraysUpgradeable.unsafeSetLength(self._zeros, treeDepth);

        // Build each root of zero-filled subtrees
        bytes32 currentZero = zero;
        for (uint256 i = 0; i < treeDepth; ++i) {
            ArraysUpgradeable.unsafeAccess(self._zeros, i).value = currentZero;
    ...
    function push(Bytes32PushTree storage self, bytes32 leaf) internal returns (uint256 index, bytes32 newRoot) {
        return push(self, leaf, HashesUpgradeable.commutativeKeccak256);
    }
    ...
    function push(
        ...
        if (index >= 1 << treeDepth) {
            PanicUpgradeable.panic(PanicUpgradeable.RESOURCE_ERROR);
        }
        ...
        for (uint256 i = 0; i < treeDepth; i++) {
            ...
            if (isLeft) {
                ArraysUpgradeable.unsafeAccess(self._sides, i).value = currentLevelHash;
            }
            ...
            currentLevelHash = fnHash(
                isLeft ? currentLevelHash : ArraysUpgradeable.unsafeAccess(self._sides, i).value,
                isLeft ? ArraysUpgradeable.unsafeAccess(self._zeros, i).value : currentLevelHash
            );
    ...
    function update(
        ...
        return update(self, index, oldValue, newValue, proof, HashesUpgradeable.commutativeKeccak256);
    }
        ...
        unchecked {
            ...
            for (uint32 i = 0; i < treeDepth; i++) {
                ...
                if (isLeft && currentIndex == lastIndex) {
                    StorageSlotUpgradeable.Bytes32Slot storage side = ArraysUpgradeable.unsafeAccess(sides, i);
