 ...
 */
library CircularBufferUpgradeable {
    /**
    ...
    function setup(Bytes32CircularBuffer storage self, uint256 size) internal {
        ...
        clear(self);
        ArraysUpgradeable.unsafeSetLength(self._data, size);
    }
    ...
    function push(Bytes32CircularBuffer storage self, bytes32 value) internal {
        ...
        ArraysUpgradeable.unsafeAccess(self._data, index % modulus).value = value;
    }
    ...
    function count(Bytes32CircularBuffer storage self) internal view returns (uint256) {
        return MathUpgradeable.min(self._count, self._data.length);
    }
    ...
    function last(Bytes32CircularBuffer storage self, uint256 i) internal view returns (bytes32) {
        uint256 index = self._count;
        uint256 modulus = self._data.length;
        uint256 total = MathUpgradeable.min(index, modulus); // count(self)
        if (i >= total) {
            PanicUpgradeable.panic(PanicUpgradeable.ARRAY_OUT_OF_BOUNDS);
        }
        return ArraysUpgradeable.unsafeAccess(self._data, (index - i - 1) % modulus).value;
    }
    ...
    function includes(Bytes32CircularBuffer storage self, bytes32 value) internal view returns (bool) {
        ...
        for (uint256 i = 0; i < total; ++i) {
            if (ArraysUpgradeable.unsafeAccess(self._data, (index - i - 1) % modulus).value == value) {
                return true;
            }
        }
        return false;
