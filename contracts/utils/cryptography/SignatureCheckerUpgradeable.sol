...
library SignatureCheckerUpgradeable {
    ...
    function isValidSignatureNow(address signer, bytes32 hash, bytes memory signature) internal view returns (bool) {
        if (signer.code.length == 0) {
            (address recovered, ECDSAUpgradeable.RecoverError err, ) = ECDSAUpgradeable.tryRecover(hash, signature);
            return err == ECDSAUpgradeable.RecoverError.NoError && recovered == signer;
        } else {
    ...
    function isValidERC1271SignatureNow(
        ...
        bytes4 selector = IERC1271Upgradeable.isValidSignature.selector;
    ...
    function isValidSignatureNow(
        ...
        if (signer.length < 20) {
            ...
            (bool success, bytes memory result) = address(bytes20(signer)).staticcall(
                abi.encodeCall(IERC7913SignatureVerifierUpgradeable.verify, (signer.slice(20), hash, signature))
            );
            return (success &&
                result.length >= 32 &&
                abi.decode(result, (bytes32)) == bytes32(IERC7913SignatureVerifierUpgradeable.verify.selector));
        }
    }
