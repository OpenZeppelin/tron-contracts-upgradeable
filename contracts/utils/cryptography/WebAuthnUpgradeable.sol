...
library WebAuthnUpgradeable {
    ...
    function _validateChallenge(
        ...
        // solhint-disable-next-line quotes
        string memory expectedChallenge = string.concat('"challenge":"', Base64Upgradeable.encodeURL(challenge), '"');
        string memory actualChallenge = string(
            BytesUpgradeable.slice(
                bytes(clientDataJSON),
                challengeIndex,
                MathUpgradeable.saturatingAdd(challengeIndex, bytes(expectedChallenge).length)
            )
        );

        return StringsUpgradeable.equal(actualChallenge, expectedChallenge);
    }
