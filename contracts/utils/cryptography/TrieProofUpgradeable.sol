...
import {RLPUpgradeable} from "../RLPUpgradeable.sol";
 ...
 */
library TrieProofUpgradeable {
    using BytesUpgradeable for *;
    using RLPUpgradeable for *;
    ...
    function tryTraverse(
        ...
        MemoryUpgradeable.Pointer fmp = MemoryUpgradeable.getFreeMemoryPointer();
        ...
        for (uint256 i = 0; i < proof.length; ++i) {
            ...
            for (MemoryUpgradeable.Slice[] memory decoded = encoded.decodeList(); ; ) {
                if (decoded.length == BRANCH_NODE_LENGTH) {
                    ...
                    if (keyIndex == keyExpanded.length) {
                        ...
                        MemoryUpgradeable.Slice childNode = decoded[uint8(branchKey)];
                    ...
                    MemoryUpgradeable.Slice keyRemainder = keyExpanded.asSlice().slice(keyIndex); // Remaining key to match
                    MemoryUpgradeable.Slice pathRemainder = path.asSlice().slice(2 - (prefix % 2)); // Path after the prefix
                    ...
                    if (prefix <= uint8(Prefix.EXTENSION_ODD)) {
                        ...
                        MemoryUpgradeable.Slice childNode = decoded[1];
                        (currentNodeId, currentNodeIdLength) = _getNodeId(childNode);
            ...
            MemoryUpgradeable.unsafeSetFreeMemoryPointer(fmp);
        }
     ...
     * ====
     */
    function _getNodeId(MemoryUpgradeable.Slice node) private pure returns (bytes32 nodeId, uint256 nodeIdLength) {
    ...
    function _match(MemoryUpgradeable.Slice slice, bytes[] memory array, uint256 index) private pure returns (bool) {
