...
import {CalldataUpgradeable} from "../CalldataUpgradeable.sol";
import {HashesUpgradeable} from "./HashesUpgradeable.sol";
 ...
 */
library ERC7739UtilsUpgradeable {
    /**
        ...
        returns (bytes calldata signature, bytes32 appSeparator, bytes32 contentsHash, string calldata contentsDescr)
    {
        unchecked {
            ...
            if (sigLength < 66) return (CalldataUpgradeable.emptyBytes(), 0, 0, CalldataUpgradeable.emptyString());
            ...
            if (sigLength < 66 + contentsDescrLength) return (CalldataUpgradeable.emptyBytes(), 0, 0, CalldataUpgradeable.emptyString());
    ...
    function personalSignStructHash(bytes32 contents) internal pure returns (bytes32) {
        return HashesUpgradeable.efficientKeccak256(PERSONAL_SIGN_TYPEHASH, contents);
    }
    ...
    function decodeContentsDescr(
        ...
        return (CalldataUpgradeable.emptyString(), CalldataUpgradeable.emptyString());
    }
