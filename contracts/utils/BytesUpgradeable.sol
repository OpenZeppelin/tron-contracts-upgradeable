...
library BytesUpgradeable {
    ...
    function lastIndexOf(bytes memory buffer, bytes1 s, uint256 pos) internal pure returns (uint256) {
        unchecked {
            uint256 length = buffer.length;
            for (uint256 i = MathUpgradeable.min(MathUpgradeable.saturatingAdd(pos, 1), length); i > 0; --i) {
    ...
    function slice(bytes memory buffer, uint256 start, uint256 end) internal pure returns (bytes memory) {
        // sanitize
        end = MathUpgradeable.min(end, buffer.length);
    ...
    function replace(
        ...
        // sanitize
        pos = MathUpgradeable.min(pos, buffer.length);
        offset = MathUpgradeable.min(offset, replacement.length);
        length = MathUpgradeable.min(length, MathUpgradeable.min(replacement.length - offset, buffer.length - pos));
    ...
    function clz(bytes memory buffer) internal pure returns (uint256) {
        for (uint256 i = 0; i < buffer.length; i += 0x20) {
            ...
            if (chunk != bytes32(0)) {
                return MathUpgradeable.min(8 * i + MathUpgradeable.clz(uint256(chunk)), 8 * buffer.length);
            }
        }
