...
import {AccumulatorsUpgradeable} from "./structs/AccumulatorsUpgradeable.sol";
import {BytesUpgradeable} from "./BytesUpgradeable.sol";
import {MemoryUpgradeable} from "./MemoryUpgradeable.sol";
 ...
 * ====
 */
library RLPUpgradeable {
    using AccumulatorsUpgradeable for *;
    using BytesUpgradeable for *;
    using MemoryUpgradeable for *;
    ...
    function encode(uint256 input) internal pure returns (bytes memory result) {
        if (input < SHORT_OFFSET) {
            ...
            uint256 length = MathUpgradeable.log256(input) + 1;
            assembly ("memory-safe") {
                result := mload(0x40)
    ...
    function _encode(bytes memory input, uint256 offset) private pure returns (bytes memory result) {
        ...
        if (length <= SHORT_THRESHOLD) {
            ...
            uint256 lenlength = MathUpgradeable.log256(length) + 1;
            assembly ("memory-safe") {
                result := mload(0x40)
     ...
     */
    function readBool(MemoryUpgradeable.Slice item) internal pure returns (bool) {
     ...
     * ====
     */
    function readAddress(MemoryUpgradeable.Slice item) internal pure returns (address) {
     ...
     * ====
     */
    function readUint256(MemoryUpgradeable.Slice item) internal pure returns (uint256) {
        uint256 length = item.length();
     ...
     */
    function readBytes32(MemoryUpgradeable.Slice item) internal pure returns (bytes32) {
        return bytes32(readUint256(item));
    }

    /// @dev Decodes an RLP encoded bytes. See {encode-bytes}
    function readBytes(MemoryUpgradeable.Slice item) internal pure returns (bytes memory) {
    ...
    function readString(MemoryUpgradeable.Slice item) internal pure returns (string memory) {
     ...
     */
    function readList(MemoryUpgradeable.Slice item) internal pure returns (MemoryUpgradeable.Slice[] memory list) {
        ...
        for (uint256 currentOffset = listOffset; currentOffset < itemLength; ptr += 0x20) {
            ...
            MemoryUpgradeable.Slice element = item.slice(currentOffset, elementLength + elementOffset);
     ...
     * modification of the input buffer may cause the output result to become invalid.
     */
    function decodeList(bytes memory value) internal pure returns (MemoryUpgradeable.Slice[] memory) {
     ...
     */
    function _decodeLength(MemoryUpgradeable.Slice item) private pure returns (uint256, uint256, ItemType) {
        uint256 itemLength = item.length();
