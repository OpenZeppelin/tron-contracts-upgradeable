 ...
 * addresses.
 */
library InteroperableAddressUpgradeable {
    ...
    function tryParseV1Calldata(
        ...
        unchecked {
            if (self.length < 0x06) return (false, 0x0000, CalldataUpgradeable.emptyBytes(), CalldataUpgradeable.emptyBytes());

            bytes2 version = _readBytes2Calldata(self, 0x00);
            if (version != bytes2(0x0001)) return (false, 0x0000, CalldataUpgradeable.emptyBytes(), CalldataUpgradeable.emptyBytes());

            uint256 chainReferenceLength = uint8(self[0x04]);
            if (self.length < 0x06 + chainReferenceLength)
                return (false, 0x0000, CalldataUpgradeable.emptyBytes(), CalldataUpgradeable.emptyBytes());
    ...
    function _toChainReference(uint256 chainid) private pure returns (bytes memory) {
        unchecked {
            ...
            uint256 length = MathUpgradeable.log256(chainid) + 1;
