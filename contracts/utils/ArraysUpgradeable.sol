...
import {ComparatorsUpgradeable} from "./ComparatorsUpgradeable.sol";
import {SlotDerivationUpgradeable} from "./SlotDerivationUpgradeable.sol";
...
library ArraysUpgradeable {
    ...
    function sort(uint256[] memory array) internal pure returns (uint256[] memory) {
        sort(array, ComparatorsUpgradeable.lt);
        return array;
    }
    ...
    function sort(address[] memory array) internal pure returns (address[] memory) {
        sort(_castToUint256Array(array), ComparatorsUpgradeable.lt);
        return array;
    }
    ...
    function slice(address[] memory array, uint256 start, uint256 end) internal pure returns (address[] memory) {
        // sanitize
        end = MathUpgradeable.min(end, array.length);
        start = MathUpgradeable.min(start, end);
    ...
    function replace(
        ...
        // sanitize
        pos = MathUpgradeable.min(pos, array.length);
        offset = MathUpgradeable.min(offset, replacement.length);
        length = MathUpgradeable.min(length, MathUpgradeable.min(replacement.length - offset, array.length - pos));
     ...
     * WARNING: Only use if you are certain `pos` is lower than the array length.
     */
    function unsafeAccess(bytes[] storage arr, uint256 pos) internal pure returns (StorageSlotUpgradeable.BytesSlot storage) {
        bytes32 slot;
        assembly ("memory-safe") {
            slot := arr.slot
        }
     ...
     * WARNING: Only use if you are certain `pos` is lower than the array length.
     */
    function unsafeAccess(string[] storage arr, uint256 pos) internal pure returns (StorageSlotUpgradeable.StringSlot storage) {
        bytes32 slot;
        assembly ("memory-safe") {
            slot := arr.slot
        }
