...
library MemoryUpgradeable {
    ...
    function slice(Slice self, uint256 offset) internal pure returns (Slice) {
        if (offset > length(self)) PanicUpgradeable.panic(PanicUpgradeable.ARRAY_OUT_OF_BOUNDS);
    ...
    function slice(Slice self, uint256 offset, uint256 len) internal pure returns (Slice) {
        if (offset + len > length(self)) PanicUpgradeable.panic(PanicUpgradeable.ARRAY_OUT_OF_BOUNDS);
    ...
    function load(Slice self, uint256 offset) internal pure returns (bytes32 value) {
        uint256 outOfBoundBytes = MathUpgradeable.saturatingSub(0x20 + offset, length(self));
        if (outOfBoundBytes > 0x1f) PanicUpgradeable.panic(PanicUpgradeable.ARRAY_OUT_OF_BOUNDS);
    ...
    function toBytes(Slice self) internal pure returns (bytes memory result) {
        ...
        MemoryUpgradeable.Pointer ptr = _pointer(self);
        assembly ("memory-safe") {
            result := mload(0x40)
            mstore(result, len)
    ...
    function equal(Slice a, Slice b) internal pure returns (bool result) {
        MemoryUpgradeable.Pointer ptrA = _pointer(a);
        MemoryUpgradeable.Pointer ptrB = _pointer(b);
    ...
    function isReserved(Slice self) internal pure returns (bool result) {
        MemoryUpgradeable.Pointer fmp = getFreeMemoryPointer();
        MemoryUpgradeable.Pointer end = forward(_pointer(self), length(self));
        assembly ("memory-safe") {
     ...
     * correct.
     */
    function _asSlice(uint256 len, MemoryUpgradeable.Pointer ptr) private pure returns (Slice result) {
        assembly ("memory-safe") {
    ...
    function _pointer(Slice self) private pure returns (MemoryUpgradeable.Pointer result) {
        assembly ("memory-safe") {
